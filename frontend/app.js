...
async function startCameraScan() {
  if (!('BarcodeDetector' in window)) {
    return toast(
      'Dieser Browser unterstützt den eingebauten Scanner nicht. Nutze die normale Handy-Kamera oder gib die ID ein.'
    );
  }
  ...
  try {
    ...
    activeScanTimer = setInterval(async () => {
      ...
      try {
        id = new URL(raw).searchParams.get('id') || raw;
      } catch {
        id = raw;
      }
