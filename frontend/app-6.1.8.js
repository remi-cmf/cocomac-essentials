...
function qrPrintQuantityForCode(code) {
  ...
  const saved = Math.floor(Number(qrPrintQuantities.get(key) || 1)) || 1;
  return Math.max(1, Math.min(max, saved));
}
...
function renderQrDatabase() {
  ...
  wrap.innerHTML = filtered.map(code => {
    ...
    const printControl = code.productId ? `<label class="qr-print-quantity" title="Wie oft soll dieses Etikett im Download vorkommen?"><span>Druck</span><input type="number" min="1" max="${printMax}" step="1" value="${printQty}" data-qr-print-qty="${escapeHtml(code.qrCode)}" aria-label="Druckmenge für ${escapeHtml(code.productId)}"><small>/ ${printMax}</small></label>` : `<span class="qr-print-quantity is-fixed"><span>Druck</span><b>1</b></span>`;
  ...
  $$('[data-qr-print-qty]').forEach(input => {
    input.onchange = input.oninput = () => {
      ...
      if (!code) return;
      const max = qrPrintMaxForCode(code);
      let qty = Math.floor(Number(input.value || 1)) || 1;
      qty = Math.max(1, Math.min(max, qty));
      input.value = String(qty);
      qrPrintQuantities.set(key, qty);
      // Sobald eine Druckmenge bewusst gewählt wird, gehört der Code zur Auswahl.
      qrSelectedCodes.add(key);
      const checkbox = input.closest('.qr-database-row')?.querySelector('[data-select-qr]');
      if (checkbox) checkbox.checked = true;
      input.closest('.qr-database-row')?.classList.add('is-selected');
      updateQrSelectionUi();
    };
  });
...
async function downloadQrCodesAsPdf(codes, filenameBase='cocomac-qr-codes') {
  ...
  try {
    ...
    // V6.1.8: kompakte, symmetrische Schneide-Etiketten mit frei wählbarer Druckmenge pro Produkt.
