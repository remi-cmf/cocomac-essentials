...
async function downloadQrCodesAsPdf(codes, filenameBase='cocomac-qr-codes') {
  ...
  if (!window.jspdf?.jsPDF) return toast('PDF-Bibliothek konnte nicht geladen werden.');
  showActionLoader('QR-Dokument wird erstellt …', `${codes.length} QR-Codes werden für A4 vorbereitet.`);
  try {
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation:'portrait', unit:'mm', format:'a4', compress:true });
    // 30 mm QR-Grafik, etwa Daumengröße. 5 Spalten x 8 Reihen = 40 Codes pro A4-Seite.
    const pageW = 210, pageH = 297;
    const cols = 5, rows = 8, perPage = cols * rows;
    const marginX = 7.5, marginY = 7.5;
    const cellW = (pageW - marginX * 2) / cols;
    const cellH = (pageH - marginY * 2) / rows;
    const qrSize = Math.min(30, cellW - 3, cellH - 5.5);
    const productById = new Map(catalog.map(item => [String(item.id), item]));
    for (let i=0; i<codes.length; i++) {
      ...
      const x = x0 + (cellW - qrSize) / 2;
      const y = y0 + 0.6;
      const code = codes[i];
      const canvas = createQrCanvasForText(qrTargetUrlForDatabaseCode(code), 420);
      ...
      pdf.text(idText, x0 + cellW/2, y + qrSize + 2.2, { align:'center', maxWidth:cellW-2 });
      const product = productById.get(String(code.productId || ''));
      if (product?.name) {
        pdf.setFont('helvetica','normal'); pdf.setFontSize(4.4);
        const name = String(product.name).length > 28 ? String(product.name).slice(0,27) + '…' : String(product.name);
        pdf.text(name, x0 + cellW/2, y + qrSize + 4.2, { align:'center', maxWidth:cellW-2 });
      }
    }
    const date = new Date().toISOString().slice(0,10);
    pdf.save(`${filenameBase}-${date}.pdf`);
