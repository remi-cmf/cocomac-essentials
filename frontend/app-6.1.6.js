...
async function downloadQrCodesAsPdf(codes, filenameBase='cocomac-qr-codes') {
  ...
  try {
    ...
    const cols = 5, rows = 8, perPage = cols * rows;
    const marginX = 38, marginY = 38;
    const cellW = (pagePxW - marginX * 2) / cols;
    const cellH = (pagePxH - marginY * 2) / rows;
    const qrPx = Math.floor(Math.min(177, cellW - 24, cellH - 48)); // ca. 30 mm auf A4
    const productById = new Map(catalog.map(item => [String(item.id), item]));
    const pages = [];
    ...
    for (let pageStart = 0; pageStart < codes.length; pageStart += perPage) {
      ...
      for (let j = 0; j < chunk.length; j++) {
        ...
        const x = Math.round(x0 + (cellW - qrPx) / 2), y = Math.round(y0 + 4);
        ...
        ctx.fillStyle = '#111'; ctx.font = '700 18px Arial, sans-serif';
        ctx.fillText(idText, Math.round(x0 + cellW / 2), y + qrPx + 8, cellW - 10);

        const product = productById.get(String(code.productId || ''));
        if (product?.name) {
          const name = String(product.name).length > 28 ? String(product.name).slice(0, 27) + '…' : String(product.name);
          ctx.fillStyle = '#444'; ctx.font = '14px Arial, sans-serif';
          ctx.fillText(name, Math.round(x0 + cellW / 2), y + qrPx + 31, cellW - 10);
        }
      }
    ...
    toast(`${codes.length} QR-Codes als PDF erstellt · bis zu ${perPage} Codes pro A4-Seite.`);
  } catch (error) {
    console.error(error);
