...
function updateQrSelectionUi() {
  ...
  const count = qrSelectedCodes.size;
  const countEl = $('#qrSelectedCount');
  if (countEl) countEl.textContent = String(count);
  const button = $('#downloadSelectedQrBtn');
  if (button) {
    button.disabled = count === 0;
    button.firstChild.textContent = count === 1 ? 'QR-Code herunterladen ' : 'Auswahl herunterladen ';
  }
}
...
async function downloadSelectedQrCodes() {
  ...
  if (selected.length === 1) return downloadSingleDatabaseQr(selected[0]);
  return downloadQrCodesAsPdf(selected, 'cocomac-qr-auswahl');
}
...
async function downloadQrCodesAsPdf(codes, filenameBase='cocomac-qr-codes') {
  ...
  try {
    ...
    // V6.1.7: kompakte, symmetrische Schneide-Etiketten.
