...
let qrCodes = [];
let qrSelectedCodes = new Set();
let qrPrintQuantities = new Map();
...
function bind() {
  ...
  if ($('#selectVisibleQrBtn')) $('#selectVisibleQrBtn').onclick = selectVisibleQrCodes;
  if ($('#clearQrSelectionBtn')) $('#clearQrSelectionBtn').onclick = clearQrSelection;
  if ($('#downloadSelectedQrBtn')) $('#downloadSelectedQrBtn').onclick = downloadSelectedQrCodes;
  if ($('#downloadAllQrPdfBtn')) $('#downloadAllQrPdfBtn').onclick = () => downloadQrCodesAsPdf(qrCodes, 'cocomac-qr-codes-alle');
...
function qrPrintMaxForCode(code) {
  const product = catalog.find(item => String(item.id || '') === String(code?.productId || ''));
  return Math.max(1, Math.floor(Number(product?.total || 1)) || 1);
}

function qrPrintQuantityForCode(code) {
  const key = String(code?.qrCode || '');
  const max = qrPrintMaxForCode(code);
  const raw = qrPrintQuantities.has(key) ? qrPrintQuantities.get(key) : 0;
  const saved = Math.floor(Number(raw));
  return Math.max(0, Math.min(max, Number.isFinite(saved) ? saved : 0));
}

function updateQrSelectionUi() {
  // Vergessene/entfernte Codes aus der Auswahl entfernen.
  const existing = new Set(qrCodes.map(code => String(code.qrCode || '')));
  [...qrSelectedCodes].forEach(code => { if (!existing.has(code)) qrSelectedCodes.delete(code); });
  [...qrPrintQuantities.keys()].forEach(code => { if (!existing.has(code)) qrPrintQuantities.delete(code); });
  const selectedCodes = qrCodes.filter(code => qrSelectedCodes.has(String(code.qrCode || '')));
  const labelCount = selectedCodes.reduce((sum, code) => sum + qrPrintQuantityForCode(code), 0);
  const typeCount = selectedCodes.length;
  const countEl = $('#qrSelectedCount');
  if (countEl) countEl.textContent = String(labelCount);
  const button = $('#downloadSelectedQrBtn');
  if (button) {
    button.disabled = typeCount === 0;
    button.firstChild.textContent = labelCount === 1 ? 'QR-Code herunterladen ' : 'Auswahl herunterladen ';
    button.title = typeCount ? `${typeCount} QR-Code${typeCount === 1 ? '' : 's'} · ${labelCount} Etikett${labelCount === 1 ? '' : 'en'}` : '';
  }
}
...
function renderQrDatabase() {
  ...
  const productById = new Map(catalog.map(item => [String(item.id), item]));
  const filtered = getFilteredQrCodes();
  ...
  wrap.innerHTML = filtered.map(code => {
    ...
    const checked = qrSelectedCodes.has(String(code.qrCode || ''));
    const printMax = qrPrintMaxForCode(code);
    const printQty = qrPrintQuantityForCode(code);
    const printControl = code.productId ? `<label class="qr-print-quantity" title="Wie oft soll dieses Etikett im Download vorkommen?"><span>Druck</span><input type="number" min="0" max="${printMax}" step="1" value="${printQty}" data-qr-print-qty="${escapeHtml(code.qrCode)}" aria-label="Druckmenge für ${escapeHtml(code.productId)}"><small>/ ${printMax}</small></label>` : `<span class="qr-print-quantity is-fixed"><span>Druck</span><b>1</b></span>`;
    return `<div class="admin-product-row qr-database-row${checked ? ' is-selected' : ''}"><label class="qr-select-wrap" title="QR-Code auswählen"><input type="checkbox" data-select-qr="${escapeHtml(code.qrCode)}" ${checked ? 'checked' : ''} aria-label="${escapeHtml(code.qrCode)} auswählen"></label><div class="qr-database-icon">QR</div><div class="qr-database-copy"><div class="qr-code-value">${escapeHtml(code.qrCode)}</div><b>${escapeHtml(assignedText)}</b><small class="${code.status === 'frei' ? 'qr-status-free' : 'qr-status-assigned'}">${code.status === 'frei' ? 'Neu / frei' : 'Aktiv vergeben'}${dateText ? ` · ${escapeHtml(dateText)}` : ''}</small></div><div class="admin-row-actions qr-row-actions">${printControl}<button type="button" class="ghost" data-download-single-qr="${escapeHtml(code.qrCode)}">QR laden</button><button type="button" class="ghost" data-copy-qr="${escapeHtml(code.qrCode)}">Kopieren</button>${code.productId ? `<button type="button" class="ghost" data-qr-history="${escapeHtml(code.qrCode)}" data-product-id="${escapeHtml(code.productId)}">Historie</button><button type="button" class="ghost" data-open-qr-product="${escapeHtml(code.productId)}">Produkt öffnen</button>` : ''}</div></div>`;
  }).join('') || '<div class="empty-state">Für diesen Filter wurden keine QR-Codes gefunden.</div>';
  $$('[data-select-qr]').forEach(input => input.onchange = () => {
    const code = String(input.dataset.selectQr || '');
    if (input.checked) qrSelectedCodes.add(code); else qrSelectedCodes.delete(code);
    input.closest('.qr-database-row')?.classList.toggle('is-selected', input.checked);
    updateQrSelectionUi();
  });
  $$('[data-qr-print-qty]').forEach(input => {
    input.onchange = input.oninput = () => {
      const key = String(input.dataset.qrPrintQty || '');
      const code = qrCodes.find(item => String(item.qrCode || '') === key);
      if (!code) return;
      const max = qrPrintMaxForCode(code);
      ...
      qrPrintQuantities.set(key, qty);
      // 0 bedeutet: nicht drucken. Ab 1 wird der Code automatisch ausgewählt.
      const checkbox = input.closest('.qr-database-row')?.querySelector('[data-select-qr]');
      if (qty > 0) { qrSelectedCodes.add(key); if (checkbox) checkbox.checked = true; input.closest('.qr-database-row')?.classList.add('is-selected'); }
      else { qrSelectedCodes.delete(key); if (checkbox) checkbox.checked = false; input.closest('.qr-database-row')?.classList.remove('is-selected'); }
      updateQrSelectionUi();
    };
  });
  $$('[data-download-single-qr]').forEach(button => button.onclick = () => {
    const code = qrCodes.find(item => String(item.qrCode) === String(button.dataset.downloadSingleQr));
    if (code) downloadSingleDatabaseQr(code);
  });
...
function selectVisibleQrCodes() {
  getFilteredQrCodes().forEach(code => qrSelectedCodes.add(String(code.qrCode || '')));
  renderQrDatabase();
  toast(`${getFilteredQrCodes().length} sichtbare QR-Codes ausgewählt.`);
}

function clearQrSelection() {
  qrSelectedCodes.clear();
  qrPrintQuantities.clear();
  renderQrDatabase();
}

function qrTargetUrlForDatabaseCode(code) {
  const value = String(code?.qrCode || '').trim();
  return value ? `${PRODUCT_URL_BASE}?qr=${encodeURIComponent(value)}` : '';
}

function createQrCanvasForText(text, size=512) {
  if (typeof window.QRCode === 'undefined') throw new Error('QR-Code-Bibliothek konnte nicht geladen werden.');
  const holder = document.createElement('div');
  holder.className = 'qr-download-hidden-maker';
  ...
  if (!source) { holder.remove(); throw new Error('QR-Code konnte nicht erstellt werden.'); }
...
function downloadSingleDatabaseQr(code) {
  try {
    const canvas = createQrCanvasForText(qrTargetUrlForDatabaseCode(code), 700);
    const link = document.createElement('a');
    link.href = canvas.toDataURL('image/png');
    link.download = `${String(code.qrCode || 'cocomac-qr')}${code.productId ? '-' + String(code.productId) : ''}.png`;
    document.body.appendChild(link); link.click(); link.remove();
  } catch (error) { toast(error.message || 'QR-Code konnte nicht heruntergeladen werden.'); }
}

async function downloadSelectedQrCodes() {
  const selected = qrCodes.filter(code => qrSelectedCodes.has(String(code.qrCode || '')));
  if (!selected.length) return toast('Bitte mindestens einen QR-Code auswählen.');
  const expanded = [];
  selected.forEach(code => {
    const qty = qrPrintQuantityForCode(code);
    for (let i = 0; i < qty; i++) expanded.push(code);
  });
  if (expanded.length === 1) return downloadSingleDatabaseQr(expanded[0]);
  return downloadQrCodesAsPdf(expanded, 'cocomac-qr-auswahl');
}

async function downloadQrCodesAsPdf(codes, filenameBase='cocomac-qr-codes') {
  if (!Array.isArray(codes) || !codes.length) return toast('Es sind keine QR-Codes vorhanden.');
  showActionLoader('QR-Dokument wird erstellt …', `${codes.length} QR-Codes werden für A4 vorbereitet.`);
  try {
    // Eigenständige PDF-Erzeugung ohne externe PDF-Bibliothek.
    // Jede A4-Seite wird als hochauflösendes JPEG gerendert und anschließend in ein minimales PDF eingebettet.
    const pagePxW = 1240, pagePxH = 1754;
    // V6.1.9: kompakte, symmetrische Schneide-Etiketten mit frei wählbarer Druckmenge pro Produkt.
    // 6 x 9 Felder pro A4-Seite, QR ca. 22 mm, nur die Equipment-ID darunter.
    const cols = 6, rows = 9, perPage = cols * rows;
    const marginX = 36, marginY = 34;
    const cellW = (pagePxW - marginX * 2) / cols;
    const cellH = (pagePxH - marginY * 2) / rows;
    const qrPx = 130; // ca. 22 mm bei der verwendeten A4-Auflösung
    const textGap = 7;
    const textPx = 16;
    const textLineH = 19;
    const labelContentH = qrPx + textGap + textLineH;
    const pages = [];

    for (let pageStart = 0; pageStart < codes.length; pageStart += perPage) {
      const canvas = document.createElement('canvas');
      canvas.width = pagePxW; canvas.height = pagePxH;
      const ctx = canvas.getContext('2d');
      ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, pagePxW, pagePxH);
      ctx.textAlign = 'center'; ctx.textBaseline = 'top';

      const chunk = codes.slice(pageStart, pageStart + perPage);
      for (let j = 0; j < chunk.length; j++) {
        const code = chunk[j];
        const col = j % cols, row = Math.floor(j / cols);
        const x0 = marginX + col * cellW, y0 = marginY + row * cellH;
        // Gesamten Inhalt im Feld vertikal zentrieren. So ist der Weißraum
        // oben/unten sowie links/rechts zu den Nachbar-Etiketten gleichmäßig.
        const x = Math.round(x0 + (cellW - qrPx) / 2);
        const y = Math.round(y0 + (cellH - labelContentH) / 2);
        const qrCanvas = createQrCanvasForText(qrTargetUrlForDatabaseCode(code), 420);
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(qrCanvas, x, y, qrPx, qrPx);
        ctx.imageSmoothingEnabled = true;

        // Nur die eigentliche Equipment-ID, z. B. CME-KIT-002.
        // Keine CMEQR-Nummer und kein Produktname im Druckbogen.
        const idText = String(code.productId || code.qrCode || '');
        ctx.fillStyle = '#111'; ctx.font = `700 ${textPx}px Arial, sans-serif`;
        ctx.fillText(idText, Math.round(x0 + cellW / 2), y + qrPx + textGap, cellW - 10);
      }
    ...
    const pdfBlob = buildImagePagesPdf(pages, pagePxW, pagePxH);
    const url = URL.createObjectURL(pdfBlob);
    const link = document.createElement('a');
    const date = new Date().toISOString().slice(0,10);
    link.href = url; link.download = `${filenameBase}-${date}.pdf`;
    document.body.appendChild(link); link.click(); link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 3000);
    toast(`${codes.length} QR-Codes als PDF erstellt · bis zu ${perPage} kompakte Codes pro A4-Seite.`);
  } catch (error) {
    console.error(error);
    toast('QR-PDF konnte nicht erstellt werden: ' + (error.message || error));
...
function buildImagePagesPdf(jpegPages, imageW, imageH) {
  ...
  const startObj = n => { offsets[n] = length; pushText(`${n} 0 obj\n`); };
  ...
  for (let i=0;i<pageCount;i++) {
    ...
    imageObjNums.push(firstPageObj + i*3 + 1);
    contentObjNums.push(firstPageObj + i*3 + 2);
  }
  const totalObjects = 2 + pageCount*3;

  pushText('%PDF-1.4\n%\xE2\xE3\xCF\xD3\n');
  startObj(catalogObj); pushText(`<< /Type /Catalog /Pages ${pagesObj} 0 R >>\n`); endObj();
  startObj(pagesObj); pushText(`<< /Type /Pages /Kids [${pageObjNums.map(n=>`${n} 0 R`).join(' ')}] /Count ${pageCount} >>\n`); endObj();

  const pageW = 595.28, pageH = 841.89;
  for (let i=0;i<pageCount;i++) {
    const pObj=pageObjNums[i], imgObj=imageObjNums[i], cObj=contentObjNums[i];
    startObj(pObj);
    pushText(`<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 ${pageW} ${pageH}] /Resources << /XObject << /Im0 ${imgObj} 0 R >> >> /Contents ${cObj} 0 R >>\n`);
    endObj();

    const jpg = jpegPages[i];
    startObj(imgObj);
    pushText(`<< /Type /XObject /Subtype /Image /Width ${imageW} /Height ${imageH} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpg.length} >>\nstream\n`);
    pushBytes(jpg); pushText('\nendstream\n'); endObj();

    const content = `q\n${pageW} 0 0 ${pageH} 0 0 cm\n/Im0 Do\nQ\n`;
    const cb = encoder.encode(content);
    startObj(cObj); pushText(`<< /Length ${cb.length} >>\nstream\n`); pushBytes(cb); pushText('endstream\n'); endObj();
  }
