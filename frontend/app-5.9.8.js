...
async function saveProjectCalculation() {
  ...
  if (settings().cloudMode) showActionLoader('Zwischenstand wird gespeichert', 'Preise und Konditionen werden synchronisiert …');
  try {
    ...
    setCalculationSaveStatus('Speichern fehlgeschlagen');
    toast('Kalkulation konnte nicht gespeichert werden: '+error.message);
  } finally {
    buttons.forEach(button=>{button.disabled=false;button.textContent='Zwischenstand speichern';});
    if (settings().cloudMode) hideActionLoader();
  }
}
